...
이 문서의 목적은 **세션이 바뀌어도 시안 작업 흐름이 흔들리지 않도록 방식을 통일**하는 것이다. 특정 시안의 결과물이 아니라 *어떻게 작업하고 어떻게 보여주는가*만 기술한다 — 그래서 그때그때 진행 중인 시안이 무엇이든 그대로 적용된다.

레이아웃·효과·인터랙션을 확정하기 전에 **여러 시안을 한꺼번에 비교**할 때, 다음 두 축을 따른다.
...
2. **콘텐츠 모델을 먼저 확정.** 한 섹션에 성격이 다른 콘텐츠가 섞여 있으면(예: 분위기용 자동재생 영상 ↔ 정보 전달용 텍스트) **각자 자기 라벨로 분리**한다. 한쪽 텍스트가 다른 쪽 미디어를 설명하는 것처럼 읽히면 안 됨.
...
5. **역할에 맞는 과감함.** 정보 전달 영역(아카이브·목록 등)은 단정하게, 분위기 전달 영역(자동재생 영상·히어로 등)은 과감하게 — 실험적 표현 OK.
