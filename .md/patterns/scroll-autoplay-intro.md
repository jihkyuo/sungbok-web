...
4. **일방향 래치** — `introPlayed`(한번 재생되면 다시 안 함) + 재생 후 `introMax = 1` 래치. 스크롤을 되돌려도 닫히지 않음. 마운트 시 `scrollY > ε ? 1 : 0`로 초기화(중간 위치 로드는 이미 완료 상태).
